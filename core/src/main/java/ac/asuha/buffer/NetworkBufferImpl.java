...
import ac.asuha.nbt.BinaryTagReader;
import ac.asuha.nbt.BinaryTagWriter;
...
import static ac.asuha.buffer.NetworkBufferUnsafe.*;
...
final class NetworkBufferImpl implements NetworkBuffer {
    ...
    public String toString() {
        return String.format("NetworkBuffer{r%d|w%d->%d, autoResize=%s, readOnly=%s}",
                readIndex, writeIndex, capacity, autoResize != null, readOnly);
    }
    ...
    static NetworkBuffer wrap(byte @NotNull [] bytes, long readIndex, long writeIndex) {
        var buffer = new Builder(bytes.length).build();
